...
class Arg(ABC):
	...
	def name(self):
		...
		"""
		return self.name_flag.strip('-').replace('-', '_')
...
class BoolArg(Arg):
	...
	def __init__(self, *flags, default=False, negative_flags=None, help="", negative_help=None, **kw):
		...
		negative_flags : Collection[str], optional
			...
			If `...` is passed inside `negative_flags`, the auto-generated flags will be included in the list at that position.
		help : str, default=""
		...
		self.negative_flags = negative_flags if negative_flags else [...]
	...
	def add_to_ap(self, parser, **kw):
		...
		"""
		kw = self.kw | kw
		short_flags = [flag for flag in self.flags if flag[0] == '-' and flag[1] != '-']
		long_flags = [flag for flag in self.flags if flag[:2] == '--']
		dest = kw.get('dest', (long_flags[0] if long_flags else self.flags[0]).strip('-')).replace('-', '_')
		group = parser.add_mutually_exclusive_group()
		if long_flags:
			result = group.add_argument(*long_flags, dest=dest, action='store_true', help=self.yes_help, **kw)
			if ... in self.negative_flags:
				idx = self.negative_flags.index(...)
				self.negative_flags[idx:idx+1] = map(self._no_f, long_flags)
			group.add_argument(*self.negative_flags, dest=dest, action='store_false', help=self.no_help, **kw)
		if short_flags:
			result = group.add_argument(*short_flags, dest=dest, nargs='?', default=self.default, const=not self.default, type=self._str_to_bool, help=self.short_help, **kw)
		return result
